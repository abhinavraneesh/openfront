...
const BASE_OPTIONS: MissionOption[] = [
  ...
  },
  {
    ...
    needsTarget: true,
    targetingLabel: "Select ocean minefield",
  ...
  },
  {
    ...
    needsTarget: true,
    targetingLabel: "Select ocean sweep area",
    onlyFor: [UnitType.Minelayer],
    oceanOnly: true,
  },
];
...
export class FleetPanel extends LitElement implements Layer {
  ...
  private startTargeting(ship: UnitView, opt: MissionOption) {
    ...
    const range = opt.rangeFromShip
      ? (this.game.config().unitInfo(ship.type()).range ?? undefined)
      : undefined;
    ...
    this.eventBus.emit(
      new StartTargetingModeEvent(
        ...
        range,
        range === undefined ? undefined : originTile,
